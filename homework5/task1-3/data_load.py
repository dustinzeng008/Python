    ...
    for dirName, subdirList, fileList in os.walk(start_dir):
        for single_file in fileList:
            if fnmatch.fnmatch(single_file, "*log") or fnmatch.fnmatch(single_file, "*txt"):
                f = open(os.path.join(dirName, single_file))
                filePath = os.path.abspath(os.path.join(dirName, single_file))
                lastModifiedTime = os.path.getmtime(
                    os.path.join(dirName, single_file))
                fileSize = os.path.getsize(os.path.join(dirName, single_file))
                myTuple = (filePath, f.read(), lastModifiedTime)
                listOfFile.append(myTuple)
    #print(unh_crawler.getCrawlerData())
    listOfFile += unh_crawler.getCrawlerData()
    f = open(output_file, "bw")
    pickle.dump(listOfFile, f)
    f.close()
